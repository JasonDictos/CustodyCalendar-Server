...
describe("planner.custody.Planner", function()  {
	test("Alternate3", async function() {
		// January 2021
		...
		const plan: custody.Plan[] = [
			...
			{
				// Mom gets two weekends in a row, then dad gets two weekends in a row
				entity: "mom,mom,dad,dad",
				months: "Jan-Dec",
				weekdays: "Fri-Mon",
				start: {
					timeOfDay: "17:00",
		...
		for (const event of planner) {
			...
			try {
				...
				console.log(`Failed at position ${index - 1}`)
				throw e
			}
		}
	})
